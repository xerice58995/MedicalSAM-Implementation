...
if __name__ == "__main__":
    ...
    splits = get_split(root_dir, seed=43)
    train_imgs, train_masks, val_imgs, val_masks, test_imgs, test_masks = splits

    train_dataset = KvasirDataset(
        train_imgs, train_masks, img_size=parameters["img_size"], mode="train"
    )
    val_dataset = KvasirDataset(
        val_imgs, val_masks, img_size=parameters["img_size"], mode="val"
    )
    test_dataset = KvasirDataset(
        test_imgs, test_masks, img_size=parameters["img_size"], mode="test"
    )
    ...
    loss_criterion = LossFunction().to(device)

    # ---- 之前訓練好的 best_model.pth ----
    ...
    writer = SummaryWriter(log_dir="logs/experiment_2")
    ...
    for epoch in range(parameters["num_epochs"]):
        ...
        for images, masks, bbox in loop:
            ...
            writer.add_scalar(
                "Train_Loss / Steps", loss.item(), global_step=global_step
            )
        ...
        with torch.no_grad():
            ...
            writer.add_scalar("Avg Dice / Epoch", avg_val_dice, global_step=epoch)
            ...
            if visual_image is not None:
                ...
                writer.add_image(
                    "Comparison / Epoch", comparison_img, global_step=epoch
                )

            # ----End of Epoch Processing----
            print(
                f"Epoch {epoch + 1} finished | Train Loss: {avg_train_loss:.4f} |Val Dice Loss: {avg_val_dice:.4f} |Val HD95: {avg_hd95_metric:.4f} "
            )
    ...
    with torch.no_grad():
        for images, masks, bbox in tqdm(test_loader, desc="Testing"):
            ...
            for i in range(len(np_preds)):
                ...
                test_metrics["lcc_dice"].append(
                    metrics.calculate_dice(p_img_clean, t_img)
                )
                test_metrics["lcc_hd95"].append(
                    metrics.calculate_hd95(p_img_clean, t_img)
                )
    ...
    print(
        f"Final Dice Mean: {final_dice_mean:.4f} | Final Dice Std: {final_dice_std:.4f}"
    )
    print(
